...
- A namespace owner runs `kubectl get npp`, reviews a proposal, then runs
  `kubectl label npp/<name> networkenforcer.kubewarden.io/enforce=true`. The operator
...
+----------------+                            +------------+-------------+
                              ...
                              +-----------------+-----+      | fingerprint.Generate       |
                                       ...
                                       +--------+-------------------------+----------+
                                                  ...
                                                  networkenforcer.kubewarden.io/    |  on enforce label)
                                                  enforce=true            v
                                                              +-----------+--------------+
...
6. `EnforcementReconciler` watches NPPs with a predicate that fires on create
   and on `networkenforcer.kubewarden.io/enforce` label changes. When `enforce=true`,
...
`networkenforcer.kubewarden.io`, version `v1alpha1`.
...
metadata:
  ...
  namespace: shop
  labels:
    networkenforcer.kubewarden.io/enforce: "true"   # opt-in switch
spec:
  workloadRef:
...
- **Leader election.** Off by default (`--leader-elect=false`). The lease ID
  is `6163c1ee.networkenforcer.kubewarden.io` so multiple replicas can be enabled
