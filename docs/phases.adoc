...
* *Enter*
** Promote a `WorkloadNetworkPolicyProposal` with `security.kubewarden.io/promote=monitor` (see <<transitions>>), or create a `WorkloadNetworkPolicy` whose `.spec.mode` is `monitor`.
...
* *Enter*
** Promote a proposal with `security.kubewarden.io/promote=protect`, or patch an existing `WorkloadNetworkPolicy` and set `.spec.mode: protect`.
* *Leave*
...
* *Promote a proposal*
** Set the label `security.kubewarden.io/promote=monitor` or `security.kubewarden.io/promote=protect` on the `WorkloadNetworkPolicyProposal`.
** The proposal reconciler copies the backend spec onto a new `WorkloadNetworkPolicy` in the same name and namespace, sets `.spec.mode` from the label value, and adds `security.kubewarden.io/promoted-from: <proposal-name>` so the promotion relationship is explicit.
** The proposal is then deleted.
* *Leftover proposals*
** After promotion, a scraper can recreate a proposal at the same name before caches catch up. If a `WorkloadNetworkPolicy` already exists with `security.kubewarden.io/promoted-from=<proposal-name>`, the leftover proposal is deleted. That usually happens as soon as the recreate is watched; if both objects sit idle, it is picked up on the next controller-runtime resync (defaulted to 10h).
* *Return to learning*
...
** Creating a `WorkloadNetworkPolicy` by hand *without* `security.kubewarden.io/promoted-from` does *not* remove a same-named proposal. Delete the proposal yourself in that case.
* *ValidatingAdmissionPolicy*
** The Helm chart installs a cluster-scoped `ValidatingAdmissionPolicy` (and binding) that rejects `WorkloadNetworkPolicyProposal` CREATE/UPDATE when `security.kubewarden.io/promote` is present and is set to anything other than `monitor` or `protect`. The check runs only when the label is set; proposals without the label are admitted as usual.
...
* `policy_violation_acknowledged` — emitted after a `security.kubewarden.io/acknowledge-<id>` annotation is applied and the status patch succeeds.
