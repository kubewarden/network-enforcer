...
* *Leftover proposals*
** After promotion, a scraper can recreate a proposal at the same name before caches catch up. If a `WorkloadNetworkPolicy` already exists with `security.rancher.io/promoted-from=<proposal-name>`, the leftover proposal is deleted. That usually happens as soon as the recreate is watched; if both objects sit idle, it is picked up on the next controller-runtime resync (defaulted to 10h).
* *Return to learning*
